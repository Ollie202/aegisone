...
export function renderProductHomeHtml(): string {
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>ProofRail</title><style>:root{font-family:Inter,ui-sans-serif,system-ui,sans-serif;color:#111827;background:#f7f7f5}body{margin:0}.shell{max-width:900px;margin:0 auto;padding:64px 20px}.card{background:#fff;border:1px solid #e5e7eb;border-radius:20px;padding:30px;margin:18px 0}h1{font-size:44px;letter-spacing:-.05em;margin:0 0 12px}h2{font-size:18px}p{color:#4b5563;line-height:1.65}.flow{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;background:#111827;color:#f9fafb;padding:18px;border-radius:14px;white-space:pre-wrap}.pill{display:inline-block;background:#dcfce7;color:#166534;border-radius:999px;padding:7px 11px;font-weight:800;font-size:12px}</style></head><body><main class="shell"><span class="pill">PROOFRAIL PRODUCT RUNTIME</span><h1>ProofRail</h1><p>Independently reproduce publisher artifacts and Agent Skills from declared source, preserve the evidence on 0G, and keep provenance separate from security analysis.</p><div class="card"><h2>What each layer does</h2><div class="flow">Supabase = app/job memory\nRailway = ProofRail API + workers\n0G Sandbox = independent builder/packager\n0G Storage = canonical evidence\n0G Chain = immutable commitment anchor</div></div><div class="card"><h2>Agent Skills</h2><p>Skill verification shows two independent dimensions: source-package correspondence (MATCH/MISMATCH) and deterministic static audit findings. MATCH never means safe.</p></div><div class="card"><h2>Trust rule</h2><p>Supabase may remember that a job exists, but it cannot decide MATCH or MISMATCH. Any verification page with evidence is rendered through integrity-checked ProofRail projections.</p></div></main></body></html>`;
}
