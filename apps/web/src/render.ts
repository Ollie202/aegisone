...
export function renderVerificationHtml(verification: VerificationJson): string {
  ...
  return `<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>ProofRail verification</title>
<style>
...
</style></head><body><main class="shell"><header><div class="brand">ProofRail</div><div class="badge ${statusClass}">${escapeHtml(view.verdict)}</div></header>
<div class="hero"><h1>${escapeHtml(view.headline)}</h1><p>Independent reproduction evidence for an explicit publisher source claim. The verdict comes from ProofRail core byte checks, not from this page.</p></div>
...
<p class="footer">ProofRail reports source assurance and artifact correspondence separately. MATCH means the compared bytes agree; it is not a malware or safety verdict.</p></main></body></html>`;
}
