...
function assertCommitmentsEqual(
  actual: Awaited<ReturnType<typeof readEvidence>>,
  expected: ReturnType<typeof attachM7StorageEvidence>["preparedAristotleAnchor"]["commitments"],
): void {
...
function buildArchiveFetchCommand(): string {
  const script = `
(async () => {
  ...
  fs.writeFileSync(${JSON.stringify(SOURCE_ARCHIVE)}, Buffer.from(await archiveResponse.arrayBuffer()));
...
try {
  ...
  const fetchCommand = buildArchiveFetchCommand();
  const fetchResponse = await sandboxClient.exec(createdSandboxId, fetchCommand, 60);
  assertExecSucceeded(fetchResponse, fetchCommand);
  const extractCommand = `mkdir -p ${SOURCE_PATH} && tar -xzf ${SOURCE_ARCHIVE} -C ${SOURCE_PATH} --strip-components=1`;
  const extractResponse = await sandboxClient.exec(createdSandboxId, extractCommand, 60);
  assertExecSucceeded(extractResponse, extractCommand);

  const markerBytes = await sandboxClient.downloadFile(createdSandboxId, SOURCE_MARKER);
  const resolvedCommitSha = Buffer.from(markerBytes).toString("utf8").trim();
  if (resolvedCommitSha !== SOURCE_COMMIT) throw new Error(`0G Sandbox archive resolved ${resolvedCommitSha}, expected ${SOURCE_COMMIT}`);
  output.sourceAcquisition = {
    method: "GITHUB_API_EXACT_SHA_TARBALL",
    requestedCommitSha: SOURCE_COMMIT,
    resolvedCommitSha,
    commitApiVerified: true,
    fetchResponse,
    extractResponse,
  };
  ...
  output.build = {
    ...
    sourceAcquisition: "GITHUB_API_EXACT_SHA_TARBALL",
