...
**Phase:** M8 active — backend-first verified capability discovery; M8.2 merged (PR #34); M8.3 merged (PR #35); M8.4 merged (PR #36); M8.5 implemented and locally green on its issue branch, merge gate pending; production Supabase migration application/advisor review and the GitHub App creation both pending repo-owner action
...
4. **M8.5 / Issue #24 — current:** GitHub App source authentication and canonical source claims.
...
Open/review the **M8.5 / Issue #24** pull request, require green CI, merge. Two repo-owner actions remain outstanding and are not blocking further backend issues, but are required before `REPOSITORY_AUTHENTICATED` can be proven live: (1) apply `supabase/migrations/202608260001_m8_4_capability_catalog.sql` and `supabase/migrations/202608260002_m8_5_source_claims.sql` to the production Supabase project and review the security/performance advisors; (2) create/install the `ProofRail Source Verifier` GitHub App per `docs/14-source-authentication.md` and supply `GITHUB_APP_CLIENT_ID`/`GITHUB_APP_CLIENT_SECRET`/`GITHUB_APP_SLUG`/`GITHUB_OAUTH_CALLBACK_URL`/`GITHUB_OAUTH_STATE_SECRET` to `proofrail-app` via Railway, then complete one interactive browser authorization against a real public repository. Do not begin M8.6 in this context.
