# ADR-010 — Separate capability discovery, AegisOne evidence, and consumer policy
...
M8 turns AegisOne from a verification-only surface into a capability discovery hub for humans and agents. Discovery systems can return Agent Skills, MCP servers, agents, and APIs with useful relevance metadata, but discovery itself does not prove publisher identity, source correspondence, security, or evidence freshness.

The largest product risk is accidental trust escalation: an indexed or highly relevant resource must never become `VERIFIED` merely because a catalog row, upstream registry, or UI badge says so. AegisOne already separates source identity, correspondence, and security findings; M8 must preserve and extend that separation.

## Decision

Introduce a provider-independent `@aegisone/capability-model` package with four supported resource kinds:
...
5. **Security assessment + canonical evidence** — deterministic findings and AegisOne evidence pointers/freshness, neither of which rewrites correspondence.
...
AegisOne can safely aggregate broad external discovery sources while making a narrow, strong verification claim only where evidence exists. Agent Skills can receive full AegisOne verification in M8; MCP/A2A/API resources can remain explicitly `INDEXED` or partially inspected until equivalent evidence paths are implemented.

This model also gives agent integrations a stable interface: discovery answers *what might help*, AegisOne evidence answers *what was observed*, and consumer policy answers *whether this consumer will use it*.
